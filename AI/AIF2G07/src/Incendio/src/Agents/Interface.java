		...
		public void action() {
			...
			if (msg != null && msg.getPerformative() == ACLMessage.INFORM) { // receber atualizações				
					try {
						...
						else if("info_nr_broken_vehicles".equals(ontology)) {
							graphicalinterface.nr_avarias = (int) msg.getContentObject();
						}
						
						else if("nr_not_allocated".equals(ontology)) {
							graphicalinterface.nr_incendios_nao_alocados = (int) msg.getContentObject();
						}
					}
