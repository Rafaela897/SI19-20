...
public class Station extends Agent {
	...
	protected void setup() {
		...
		try {
			engine.batch("src/jess/ressource_allocation.clp");
			engine.reset();
		...
		addBehaviour(new AlocarRecursos(this,500));
	}


	private class AlocarRecursos extends TickerBehaviour {

		public AlocarRecursos(Agent a, long period) {
			super(a, period);
		...
		public void addTaskFact(String name,Rete engine) {
			
			
			try {
				
				Fact f = new Fact("task", engine);
				f.setSlotValue("name", new Value(name, RU.STRING));
				engine.assertFact(f);
		...
		public AID allocate_ressource(int x,int y,ArrayList<DFAgentDescription> results) {
			...
			HashMap<String,DFAgentDescription> v = new HashMap<String,DFAgentDescription>();
			
			DFAgentDescription best_vehicle = null; 
			...
			for (int d = 0; d < results.size(); d++) {
				...
				DFAgentDescription vehicle = results.get(d);
					
				PosVehicle cordenada = localizacoes.get(vehicle.getName());
					
					
				if(cordenada != null) 
					v.put(vehicle.getName().toString(),vehicle);
			...
			try {
					...
					for(String key: v.keySet()) {
						...
						addVehicleFact(key,localizacoes.get(v.get(key).getName()),x,y,engine);
					}
					
					addTaskFact("task",engine);
				...
				try {
					...
					if(msg.getPerformative() == ACLMessage.AGREE) {
						...
						if("veiculo".equals(msg.getOntology())) {
							...
							System.out.println(msg.getSender().getLocalName() + " refused to help vehicle");
						...
						System.out.println("task completed by " + msg.getSender().getLocalName());
						...
						System.out.println(msg.getSender().getLocalName() + " needs help");
						nr_avarias++;
					}
...
private class UpdateInterface extends TickerBehaviour {
		...
		public void AtualizarInterface() {
			...
			try {
				...
				msg.setOntology("info_nr_broken_vehicles");
				msg.setContentObject(nr_avarias);
				send(msg);
				
				msg.setOntology("nr_not_allocated");
				msg.setContentObject(NAincendios.size());
				send(msg);
