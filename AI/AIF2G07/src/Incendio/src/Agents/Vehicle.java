...
public class Vehicle extends Agent {
	...
	protected void setup() {
		...
		try {
			engine.batch("src/jess/free_time_allocation.clp");
			engine.reset();
	...
	public void sendDistressCall() throws IOException {
		...
		System.out.println(my_name + " is sending a distress Call: Fuel: " + Fuel + " Water: " + Water);
		...
		public void onTick() {
				...
				if(State == Constants.StateRepair) {
					...
					if(destination != null && veiculo_avariado != null && Work_progress < destination.length) {
						...
						System.out.println(my_name + " answered distress call from " + aid_veiculo_avariado.getLocalName());
	...
	public class ReceberDiretivas extends CyclicBehaviour {
		public void action() {
			...
			if(message != null && message.getPerformative() == ACLMessage.CFP) {
				System.out.println(my_name + " is back in business");
				Fuel  = FuelCapacity;
				Water = WaterCapacity;
				State = Constants.StateFree;
				register();
			...
			else if (State == Constants.StateFree && message != null && message.getPerformative() == ACLMessage.PROPOSE ) {
				...
				try {
					...
					if(new_destination == null) {
						...
						reply.setOntology(message.getOntology());
